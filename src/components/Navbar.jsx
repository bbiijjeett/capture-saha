    ...
    <nav className={`${isHomePage ? 'bg-transparent' : 'bg-[#1B1C1D]'} w-full p-4 absolute z-50 transition-transform duration-300 ${showNavbar ? 'translate-y-0' : '-translate-y-full'}`}>
      <nav className="flex items-center justify-between md:pl-10 md:pr-10">
        <div className="flex items-center">
          <a href="/" className={`text-2xl  ${isHomePage ? 'text-[#1b1c1d]' : 'text-white'} font-black`}>Artisan<span className='text-red-500 text-2xl'>Crew</span>.co</a>
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          <a href="https://wa.me/919366797368">
            <button className="relative font-bold italic inline-flex h-12 overflow-hidden rounded-full p-[1px] focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
              <span className="absolute inset-[-1000%] animate-[spin_2s_linear_infinite] bg-[conic-gradient(from_90deg_at_50%_50%,#FFFF00_0%,#FF0000_50%,#FFFF00_100%)]" />
